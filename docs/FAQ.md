...
```javascript
import { EASE_OUT_CUBIC } from 'YOUR_JAVASCRIPT_FOLDER/universalsmoothscroll-ease-functions-min.js';
...
```javascript
import { isValidStepLengthCalculator } from 'YOUR_JAVASCRIPT_FOLDER/universalsmoothscroll-dev-helpers-min.js';
 ...
 */
const isValid = await isValidStepLengthCalculator(myBrokenCalculator, {
...
```javascript
import { EASE_IN_OUT_CUBIC } from 'YOUR_JAVASCRIPT_FOLDER/universalsmoothscroll-ease-functions-min.js';
...
```javascript
import { EASE_LINEAR } from 'YOUR_JAVASCRIPT_FOLDER/universalsmoothscroll-ease-functions-min.js';
...
This means that when an anchor is clicked all the containers returned by [`getAllScrollableParents`](./FunctionsAbout.md#getAllScrollableParentsFun) _(invoked on the element linked to the anchor)_ **_can_** be scrolled _(they won't if they're already correctly aligned)_.
...
## Q: Can I obtain the **_momentum-scrolling_** effect with this API ?
