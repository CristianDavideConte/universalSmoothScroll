...
describe('calcYScrollbarDimension', function () {
    function _getCurrentScrollPos(el) {
        return [uss.getScrollXCalculator(el)(), uss.getScrollYCalculator(el)()];
    }
    it('Tests the calcYScrollbarDimension method', function () {
        cy.window().then((win) => {
            ...
            cy.testFailingValues(
                uss.calcYScrollbarDimension,
                {
                    0: [constants.failingValuesAllNoUndefined, [true, false]],
                },
                ...
                const _elementWithScrollbarOnTheXAxisOriginalScrollPos = _getCurrentScrollPos(
                    _elementWithScrollbarOnTheXAxis
                );
                const _elementWithScrollbarOnTheYAxisOriginalScrollPos = _getCurrentScrollPos(
                    _elementWithScrollbarOnTheYAxis
                );
                const _elementWithScrollbarOnTheXYAxesOriginalScrollPos = _getCurrentScrollPos(
                    _elementWithScrollbarOnTheXYAxes
                );
                ...
                expect(uss.calcYScrollbarDimension(uss.getPageScroller(win))).to.equal(0);
                ...
                expect(uss.calcYScrollbarDimension(uss.getWindowScroller(win))).to.equal(
                    uss.calcYScrollbarDimension(win)
                );
                ...
                expect(constants.arraysAreEqual(_getCurrentScrollPos(_pageScroller), _pageScrollerOriginalPos)).to.be
                    .true;
                expect(
                    constants.arraysAreEqual(
                        _getCurrentScrollPos(_noScrollbarElement),
                        _noScrollbarElementOriginalScrollPos
                    )
                ).to.be.true;
                expect(
                    constants.arraysAreEqual(
                        _getCurrentScrollPos(_elementWithScrollbarOnTheXAxis),
                        _elementWithScrollbarOnTheXAxisOriginalScrollPos
                    )
                ).to.be.true;
                expect(
                    constants.arraysAreEqual(
                        _getCurrentScrollPos(_elementWithScrollbarOnTheYAxis),
                        _elementWithScrollbarOnTheYAxisOriginalScrollPos
                    )
                ).to.be.true;
                expect(
                    constants.arraysAreEqual(
                        _getCurrentScrollPos(_elementWithScrollbarOnTheXYAxes),
                        _elementWithScrollbarOnTheXYAxesOriginalScrollPos
                    )
                ).to.be.true;

                //Test if the yielded results are the same as the uss.calcScrollbarsDimensions method
                expect(uss.calcYScrollbarDimension(_pageScroller)).to.equal(
                    uss.calcScrollbarsDimensions(_pageScroller)[1]
                );
                expect(uss.calcYScrollbarDimension(_noScrollbarElement)).to.equal(
                    uss.calcScrollbarsDimensions(_noScrollbarElement)[1]
                );
                expect(uss.calcYScrollbarDimension(_elementWithScrollbarOnTheXAxis)).to.equal(
                    uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXAxis)[1]
                );
                expect(uss.calcYScrollbarDimension(_elementWithScrollbarOnTheYAxis)).to.equal(
                    uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheYAxis)[1]
                );
                expect(uss.calcYScrollbarDimension(_elementWithScrollbarOnTheXYAxes)).to.equal(
                    uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes)[1]
                );
...
describe('calcYScrollbarDimension-webkit-scrollbar-modifiers', function () {
    ...
    it('Tests the calcYScrollbarDimension method with elements that have the ::webkit-scrollbar css modifier', function () {
        cy.window().then((win) => {
            ...
            expect(uss.calcYScrollbarDimension(_elementWithScrollbarOnTheXYAxes, true)).to.equal(
                uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes, true)[1]
            );
            expect(uss.calcYScrollbarDimension(_elementWithScrollbarOnTheXYAxes, false)).to.equal(
                uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes, false)[1]
            );
            expect(constants.arraysAreEqual(_getCurrentScrollPos(_elementWithScrollbarOnTheXYAxes), _originalScrollPos))
                .to.be.true;
