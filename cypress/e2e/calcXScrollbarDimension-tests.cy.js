    ...
    it('Tests the calcXScrollbarDimension method', function () {
        cy.window().then((win) => {
            ...
            cy.testFailingValues(
                ...
                expect(uss.calcXScrollbarDimension(uss.getWindowScroller(win))).to.equal(
                    uss.calcXScrollbarDimension(win)
                );
                ...
                expect(uss.calcXScrollbarDimension(_pageScroller)).to.equal(
                    uss.calcScrollbarsDimensions(_pageScroller)[0]
                );
                expect(uss.calcXScrollbarDimension(_noScrollbarElement)).to.equal(
                    uss.calcScrollbarsDimensions(_noScrollbarElement)[0]
                );
                expect(uss.calcXScrollbarDimension(_elementWithScrollbarOnTheXAxis)).to.equal(
                    uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXAxis)[0]
                );
                expect(uss.calcXScrollbarDimension(_elementWithScrollbarOnTheYAxis)).to.equal(
                    uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheYAxis)[0]
                );
                expect(uss.calcXScrollbarDimension(_elementWithScrollbarOnTheXYAxes)).to.equal(
                    uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes)[0]
                );
...
describe('calcXScrollbarDimension-webkit-scrollbar-modifiers', function () {
    ...
    it('Tests the calcXScrollbarDimension method with elements that have the ::webkit-scrollbar css modifier', function () {
        cy.window().then((win) => {
            ...
            expect(uss.calcXScrollbarDimension(_elementWithScrollbarOnTheXYAxes, true)).to.equal(
                uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes)[0],
                true
            );
            expect(uss.calcXScrollbarDimension(_elementWithScrollbarOnTheXYAxes, false)).to.equal(
                uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes)[0],
                false
            );
