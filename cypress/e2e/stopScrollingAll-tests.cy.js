...
describe('stopScrollingAll-containersData-integrity', function () {
    it("Checks if the stopScrollingAll function cleans the uss._containersData's arrays correctly", function () {
        cy.window().then((win) => {
            ...
            const _elements = [win, win.document.documentElement, win.document.body].concat(
                constants.getChildrenRecursively(document.body)
            );
            ...
            for (const container of _elements) {
                ...
                if (container !== win) {
                    expect(constants.arraysAreEqual(_containerData[constants.K_MCBQ], _mutationCallbacksQueue)).to.be
                        .true;
                }
            }
        });
