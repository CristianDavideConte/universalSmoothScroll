...
describe('calcScrollbarsDimensions', function () {
    ...
    it('Tests the calcScrollbarsDimensions method', function () {
        cy.window().then((win) => {
            ...
            expect(constants.arraysAreEqual(_windowScrollbarsDimensions, _windowScrollbarsCachedDimensions)).to.be.true;

            cy.testFailingValues(
                uss.calcScrollbarsDimensions,
                {
                ...
                Array.from(_head.children).forEach(
                    (el) => expect(constants.arraysAreEqual(uss.calcScrollbarsDimensions(el), [0, 0])).to.be.true
                );

                //Test the scrollbars' dimensions.
                expect(constants.arraysAreEqual(uss.calcScrollbarsDimensions(uss.getPageScroller(win)), [_maxDim, 0]))
                    .to.be.true;
                expect(constants.arraysAreEqual(uss.calcScrollbarsDimensions(_noScrollbarElement), [0, 0])).to.be.true;
                expect(
                    constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXAxis), [
                        _maxDim,
                        0,
                    ])
                ).to.be.true;
                expect(
                    constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheYAxis), [
                        0,
                        _maxDim,
                    ])
                ).to.be.true;
                expect(
                    constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes), [
                        _maxDim,
                        _maxDim,
                    ])
                ).to.be.true;
                expect(
                    constants.arraysAreEqual(
                        uss.calcScrollbarsDimensions(uss.getWindowScroller(win)),
                ...
                expect(constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes), _dims))
                    .to.be.true;
            });
        });
    });
...
describe('calcScrollbarsDimensions-webkit-scrollbar-modifiers', function () {
    ...
    it('Tests the calcScrollbarsDimensions method with elements that have the ::webkit-scrollbar css modifier', function () {
        cy.window().then((win) => {
            ...
            expect(
                constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes, false), [
                    _maxDim,
                    _maxDim,
                ])
            ).to.be.true;
            expect(
                constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes, true), [0, 0])
            ).to.be.true;
            expect(constants.arraysAreEqual(_getCurrentScrollPos(_elementWithScrollbarOnTheXYAxes), _originalScrollPos))
                .to.be.true;
            ...
            expect(
                constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes, false), [0, 0])
            ).to.be.true;
            expect(
                constants.arraysAreEqual(uss.calcScrollbarsDimensions(_elementWithScrollbarOnTheXYAxes, true), [
                    0,
                    _maxDim,
                ])
            ).to.be.true;
