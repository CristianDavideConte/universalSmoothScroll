    ...
    it('Tests the getXScrollableParent method', function () {
        cy.window().then((win) => {
            cy.testFailingValues(
                ...
                expect(uss.getXScrollableParent(_positionAbsoluteElementDirectBodyChild)).to.equal(_bodyParent);
                ...
                expect(uss.getXScrollableParent(_positionAbsoluteElementB)).to.equal(_htmlParent);
                ...
                expect(uss.getXScrollableParent(_genericElement4, false)).to.equal(_body);
                expect(uss.getXScrollableParent(_genericElement4, true)).to.equal(_body);
                expect(uss.getXScrollableParent(_genericElement5, false)).to.equal(_body);
                expect(uss.getXScrollableParent(_genericElement5, true)).to.equal(_genericElementParent5);
            });
        });
    });
});
