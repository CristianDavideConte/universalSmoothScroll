...
describe('calcBordersDimensions', function () {
    it('Tests the calcBordersDimensions method', function () {
        cy.window().then((win) => {
            ...
            expect(constants.arraysAreEqual(_windowBordersDimensions, _windowBordersCachedDimensions)).to.be.true;
            ...
            Array.from(_head.children).forEach(
                (el) => expect(constants.arraysAreEqual(uss.calcBordersDimensions(el), [0, 0, 0, 0])).to.be.true
            );

            cy.testFailingValues(
                uss.calcBordersDimensions,
                {
                ...
                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement), [0, 0, 0, 0])).to.be.true;

                _borderedElement.classList.add('all-border');

                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, false), [0, 0, 0, 0])).to.be
                    .true;
                expect(
                    constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, true), [
                        _maxDim,
                        _maxDim,
                        _maxDim,
                        _maxDim,
                ...
                expect(
                    constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, false), [
                        _maxDim,
                        _maxDim,
                        _maxDim,
                        _maxDim,
                ...
                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, true), [_maxDim, 0, 0, 0]))
                    .to.be.true;

                _borderedElement.classList.remove('top-border');
                _borderedElement.classList.add('right-border');

                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, false), [_maxDim, 0, 0, 0]))
                    .to.be.true;
                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, true), [0, _maxDim, 0, 0]))
                    .to.be.true;

                _borderedElement.classList.remove('right-border');
                _borderedElement.classList.add('bottom-border');

                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, false), [0, _maxDim, 0, 0]))
                    .to.be.true;
                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, true), [0, 0, _maxDim, 0]))
                    .to.be.true;

                _borderedElement.classList.remove('bottom-border');
                _borderedElement.classList.add('left-border');

                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, false), [0, 0, _maxDim, 0]))
                    .to.be.true;
                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, true), [0, 0, 0, _maxDim]))
                    .to.be.true;

                _borderedElement.classList.remove('left-border');
                _borderedElement.classList.add('no-border');

                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, false), [0, 0, 0, _maxDim]))
                    .to.be.true;
                expect(constants.arraysAreEqual(uss.calcBordersDimensions(_borderedElement, true), [0, 0, 0, 0])).to.be
                    .true;
