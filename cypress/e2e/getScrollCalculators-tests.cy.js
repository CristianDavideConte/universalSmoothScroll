        ...
        cy.window().then((win) => {
            cy.testFailingValues(
                ...
                cy.waitForUssCallback((resolve) => {
                    _elements.forEach((el) => {
                        uss.scrollTo(Math.random() * el.scrollWidth, Math.random() * el.scrollHeight, el, () => {
                            const [_scrollXCalculator, _scrollYCalculator] = uss.getScrollCalculators(el);

                            result = _scrollXCalculator() === el.scrollLeft && _scrollYCalculator() === el.scrollTop;
